...
def _find_course_by_name(db, user_id: int, name: str):
    from app.backend.services.course_service import get_user_courses
...
def search_materials(course_name: str, config: RunnableConfig) -> str:
    ...
    from app.backend.services.material_service import get_course_materials
    db = _get_db()
    try:
...
def add_material(course_name: str, title: str, content: str, config: RunnableConfig) -> str:
    ...
    from app.backend.services.material_service import create_material
    db = _get_db()
    try:
        ...
        # 文本分块 + 向量化，使资料立即可语义检索
        from app.backend.utils.text_chunker import build_material_chunks
        from app.backend.services.material_chunk_service import replace_material_chunks
        from app.backend.services.vector_service import index_material_vectors
